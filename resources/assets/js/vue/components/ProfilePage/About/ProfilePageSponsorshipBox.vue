<template>
  <div class="ProfilePageSponsorshipBox pp-sp-box box">
    <div class="pp-sp-box__header">
      <div class="pp-sp-box__left">
        <div class="pp-sp-box__header-title">
          <template v-if="isMine">
            {{ trans('profile_page.my_sponsorship') }}
          </template>
          <template v-else>
            {{ user.displayName || user.display_name }}{{ trans('profile_page.s_sponsorship') }}
          </template>
        </div>
      </div>
    ...
    <div class="pp-sp-box__main">
      <div
        ...
        <div
          v-for="(sponsorship, index) in mySponsorship"
          v-if="index < 6"
          :key="index"
          ...
          >
            <div slot="content">
              ...
              <div class="pp-sp-box__tooltip-sub-title">
                {{ trans('profile_page.donated') }}: ${{ sponsorship.donated }}
              </div>
            </div>
            <div
              :style="{
                'background-image': `url(${sponsorship.avatar})`,
                'height': `${size}px`,
                'width': `${size}px`,
              }"
              class="pp-sp-box__avatar"
              @click="goToSponsorship(sponsorship)"
            />
          </el-tooltip>
        </div>
      </div>
...
export default {
  name: 'ProfilePageSponsorshipBox',
  components: {},
  props: {
    user: {
      type: Object,
  ...
  computed: {
    ...
    mySponsorship() {
      const sponsors = this.user.sponsorship.concat();
      return sponsors.splice(0, 6)
        .reverse();
    },
  },
  mounted() {
