  ...
  >
    ...
    >
      ...
      <div class="gmg-item__main">
        <div
          :class="[
            'gmg-item__preview-btn',
            {'gmg-item__preview-btn--play': previewIcon === 'play' || previewIcon === 'pause'}
          ]"
...
export default {
  ...
  computed: {
    ...
    previewIcon() {
      ...
      if (has(this.item, '__typename')) {
        const typename = String(this.item.__typename).toUpperCase();

        if (typename === constants.VIDEO || typename === constants.AUDIO) {
          icon = this.isPlaying ? 'pause' : 'play';
        }
        if (typename === constants.IMAGE) {
          icon = 'eye';
        }
      }
